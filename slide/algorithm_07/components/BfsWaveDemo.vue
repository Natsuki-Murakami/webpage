...
const stepItems = [
  ...
  { id: 2, text: 'Step 2(隣接点列挙)' },
  { id: 3, text: 'Step 3(一斉訪問)' },
]
...
const cloneFrame = (activeSteps, frontier, visited, previewNodes, justVisited, layerEdges, candidateEdges) => ({
  activeSteps,
  frontier: [...frontier].sort(compareNodeId),
  visited: [...visited].sort(compareNodeId),
  previewNodes: [...previewNodes].sort(compareNodeId),
  justVisited: [...justVisited].sort(compareNodeId),
...
const buildFrames = (adjacency) => {
  ...
  const frames = [cloneFrame([0, 1], frontier, visited, [], [], layerEdges, [])]
  ...
  while (true) {
    ...
    for (const u of currentFrontier) {
      for (const v of adjacency[u]) {
        ...
        previewNodes.add(v)
        candidateEdges.add(edgeKey(u, v))
      }
    }

    frames.push(cloneFrame([2], currentFrontier, visited, previewNodes, [], layerEdges, candidateEdges))

    if (previewNodes.size === 0) {
      frames.push(cloneFrame([2], [], visited, [], [], layerEdges, []))
      break
    }

    for (const v of previewNodes) visited.add(v)
    for (const key of candidateEdges) layerEdges.add(key)

    const nextFrontier = [...previewNodes].sort(compareNodeId)
    frames.push(cloneFrame([3], [], visited, [], nextFrontier, layerEdges, []))
...
const nodeState = (id) => {
  if (frame.value.frontier.includes(id) || frame.value.justVisited.includes(id)) return 'frontier'
...
const valueOf = (id) => (frame.value.visited.includes(id) ? 1 : 0)
</script>

<template>
  <div class="bfs-wave-demo">
    <div class="bfs-wave-toolbar">
      <div class="bfs-wave-status">
        <span>{{ index + 1 }} / {{ graph.frames.length }}</span>
      </div>
      <div class="bfs-wave-buttons">
        <button class="bfs-wave-button bfs-wave-arrow-button" @click="prev" :disabled="index === 0" aria-label="前へ">◀</button>
        <button
          class="bfs-wave-button bfs-wave-arrow-button"
          @click="next"
        ...
        <button class="bfs-wave-button" :class="{ 'bfs-wave-button-active': mode === 'connected' }" @click="setMode('connected')">連結</button>
        <button
          class="bfs-wave-button"
          :class="{ 'bfs-wave-button-active': mode === 'disconnected' }"
          @click="setMode('disconnected')"
        >
          非連結
        </button>
        <button class="bfs-wave-button bfs-wave-icon-button" @click="regenerate" aria-label="再生成">↻</button>
      </div>
    </div>

    <div class="bfs-wave-stage">
      <svg viewBox="0 0 440 255" class="bfs-wave-graph" role="img" aria-label="幅優先探索の実行例">
        <line
        ...
        <g v-for="node in graph.nodes" :key="node.id" :transform="`translate(${node.x}, ${node.y})`">
          <circle class="bfs-wave-node" :class="`bfs-wave-node-${nodeState(node.id)}`" r="20" />
          <text class="bfs-wave-node-name" text-anchor="middle" dy="-4">{{ node.id }}</text>
          <text class="bfs-wave-node-value" text-anchor="middle" dy="12">f={{ valueOf(node.id) }}</text>
        </g>
      </svg>

      <div class="bfs-wave-side">
        <div class="bfs-wave-legend">
          <div><span class="bfs-wave-swatch bfs-wave-swatch-frontier"></span>最前線の頂点</div>
          <div><span class="bfs-wave-swatch bfs-wave-swatch-visited"></span>探索済み頂点</div>
          <div><span class="bfs-wave-swatch bfs-wave-swatch-preview"></span>次に訪問する候補</div>
          <div><span class="bfs-wave-swatch bfs-wave-swatch-unvisited"></span>未探索頂点</div>
        </div>
        <div class="bfs-wave-steps">
          <div
            v-for="item in stepItems"
            :key="item.id"
            class="bfs-wave-step-item"
            :class="{ 'bfs-wave-step-item-active': frame.activeSteps.includes(item.id) }"
          >
            <span>{{ item.text }}</span>
          </div>
        </div>
...
.bfs-wave-button-active {
  background: #003366;
  color: white;
}

.bfs-wave-button:disabled {
  opacity: 0.4;
}

.bfs-wave-stage {
  display: grid;
  grid-template-columns: minmax(0, 24.1rem) 14.4rem;
  gap: 0.65rem;
  align-items: start;
  justify-content: space-between;
}
...
.bfs-wave-node-frontier {
  fill: #ffde59;
  stroke: #d17a00;
}

.bfs-wave-node-visited {
  fill: #d9ecff;
  stroke: #003366;
}

.bfs-wave-node-preview {
  fill: #fff3d9;
  stroke: #d17a00;
  stroke-dasharray: 4 3;
}

.bfs-wave-node-unvisited {
  fill: white;
  stroke: #7f8fa0;
}
...
.bfs-wave-step-item + .bfs-wave-step-item {
  margin-top: 0.18rem;
}

.bfs-wave-step-item-active {
  color: #cc0000;
  font-weight: 700;
}

.bfs-wave-legend > div + div {
  margin-top: 0.2rem;
}
...
.bfs-wave-swatch-frontier {
  background: #ffde59;
  border-color: #d17a00;
}

.bfs-wave-swatch-visited {
  background: #d9ecff;
  border-color: #003366;
}

.bfs-wave-swatch-preview {
  background: #fff3d9;
  border-color: #d17a00;
}

.bfs-wave-swatch-unvisited {
  background: white;
  border-color: #7f8fa0;
}
